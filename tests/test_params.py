...
from meshwork.runtime.params import validate_param, validate_params, resolve_params, repair_parameters, ParamError
...
def test_param_validation_all_types():
    ...
    set = ParameterSet(
        ...
        test_ramp_float=[{'pos': 0.0, 'value': 0.0, 'interp': rampBasis.Linear},
                         {'pos': 1.0, 'value': 1.0, 'interp': rampBasis.Linear}],
        test_ramp_color=[{'pos': 0.0, 'c': [1.0, 1.0, 1.0], 'interp': rampBasis.Linear},
...
def test_param_validation_ramp():
    ...
    set_bad_miss_pos = ParameterSet(
        test_ramp=[{'value': 0.0, 'interp': rampBasis.Linear}, {'pos': 1.0, 'value': 1.0, 'interp': rampBasis.Linear}])
    set_bad_miss_value = ParameterSet(
        test_ramp=[{'pos': 0.0, 'interp': rampBasis.Linear}, {'pos': 1.0, 'value': 1.0, 'interp': rampBasis.Linear}])
    set_bad_miss_interp = ParameterSet(
        test_ramp=[{'pos': 0.0, 'value': 0.0}, {'pos': 1.0, 'value': 1.0, 'interp': rampBasis.Linear}])
...
def test_param_conversion_ramp():
    ...
    set_good_pos = ParameterSet(test_ramp=[{'pos': 0, 'value': 0.0, 'interp': rampBasis.Linear},
                                           {'pos': 1, 'value': 1.0, 'interp': rampBasis.Linear}])
    set_good_val = ParameterSet(test_ramp=[{'pos': 0.0, 'value': 0, 'interp': rampBasis.Linear},
                                           {'pos': 1.0, 'value': 1, 'interp': rampBasis.Linear}])
    set_good_col = ParameterSet(test_ramp=[{'pos': 0.0, 'c': [0, 0, 0], 'interp': rampBasis.Linear},
...
def test_param_validation_ramp_float():
    ...
    spec = ParameterSpec(params={
        'test_ramp': RampParameterSpec(
            ...
            ramp_parm_type=rampParmType.Float,
            default=[{'pos': 0.0, 'value': 0.0, 'interp': rampBasis.Linear}])})
    set_good = ParameterSet(test_ramp=[{'pos': 0.0, 'value': 0.0, 'interp': rampBasis.Linear},
                                       {'pos': 1.0, 'value': 1.0, 'interp': rampBasis.Linear}])
    set_bad_pos = ParameterSet(test_ramp=[{'pos': 'd', 'value': 0.0, 'interp': rampBasis.Linear},
                                          {'pos': 'd', 'value': 0.0, 'interp': rampBasis.Linear}])
    set_bad_value = ParameterSet(test_ramp=[{'pos': 0.0, 'value': 'f', 'interp': rampBasis.Linear},
                                            {'pos': 1.0, 'value': 'bad', 'interp': rampBasis.Linear}])
    set_bad_interp = ParameterSet(
        test_ramp=[{'pos': 0.0, 'value': 0.0, 'interp': 'foo'}, {'pos': 1.0, 'value': 0.0, 'interp': 'foo'}])
    validate_params(spec, set_good)
    with pytest.raises(ParamError) as exc_info:
...
def test_param_validation_ramp_color():
    ...
    spec = ParameterSpec(params={
        'test_ramp': RampParameterSpec(
            ...
            ramp_parm_type=rampParmType.Color,
            default=[{'pos': 0.0, 'c': [1.0, 1.0, 1.0], 'interp': rampBasis.Linear}])})
    set_good = ParameterSet(test_ramp=[{'pos': 0.0, 'c': [1.0, 1.0, 1.0], 'interp': rampBasis.Linear},
                                       {'pos': 1.0, 'c': [0.0, 0.0, 0.0], 'interp': rampBasis.Linear}])
    set_bad_pos = ParameterSet(test_ramp=[{'pos': 'd', 'c': [1.0, 1.0, 1.0], 'interp': rampBasis.Linear},
                                          {'pos': 'd', 'c': [1.0, 1.0, 1.0], 'interp': rampBasis.Linear}])
    set_bad_c_outer = ParameterSet(test_ramp=[{'pos': 0.0, 'c': 'f', 'interp': rampBasis.Linear},
                                              {'pos': 1.0, 'c': 'bad', 'interp': rampBasis.Linear}])
    set_bad_c_inner_type = ParameterSet(test_ramp=[{'pos': 0.0, 'c': ['d', 'd', 'd'], 'interp': rampBasis.Linear},
                                                   {'pos': 1.0, 'c': ['d', 'd', 'd'], 'interp': rampBasis.Linear}])
    set_bad_c_inner_count = ParameterSet(test_ramp=[{'pos': 0.0, 'c': [0.0, 0.0], 'interp': rampBasis.Linear},
                                                    {'pos': 1.0, 'c': [0.0, 0.0], 'interp': rampBasis.Linear}])
    set_bad_interp = ParameterSet(test_ramp=[{'pos': 0.0, 'c': [1.0, 1.0, 1.0], 'interp': 'foo'},
...
def test_param_validation_constants():
    ...
    spec = ParameterSpec(params={
        ...
        'test_file_array': FileParameterSpec(label='test_file_array', default=['file_qfJSVuWRJvq5PmueFPxSjXsEcST'],
                                             constant=True),
    })
    set = ParameterSet()
    with pytest.raises(ParamError) as exc_info:
...
def test_get_parameter_spec_with_literal_and_union_types():
    ...
    from meshwork.models.params import _get_parameter_spec, IntParmTemplateSpec, ToggleParmTemplateSpec, \
...
def test_get_parameter_spec_with_nested_types():
    ...
    from meshwork.models.params import _get_parameter_spec, IntParmTemplateSpec, FileParameterSpec
